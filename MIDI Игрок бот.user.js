...
// ==/UserScript==
// перевод сделал *georgiygtnntg@gmail.com*
//-----------------------------------------------------------------------------------
...
const BASE_COMMANDS = [
    ["помощь (команда)", "отображает информацию о команде, но ни одна введенная команда не показывает команды"],
    ["инфо", "получить информацию об этом боте"],
    ["link", "получить ссылку для скачивания этого бота"],
    ["Отзыв", "показывает ссылку для отправки отзыва о боте разработчику"],
    ["ping", "получает время отклика в миллисекундах"]
];
const BOT_COMMANDS = [
    ["играть [MIDI URL]", "воспроизводит определенную песню (URL должен быть прямой ссылкой на MIDI-файл)"],
    ["стоп", "останавливает воспроизведение всей музыки"],
    ["пауза", "останавливает музыку в этот момент в песне"],
    ["продолжить", "воспроизводит музыку с того места, где остановилась пауза"],
    ["песня", "показывает текущую песню и в какой момент времени"],
    ["повтор", "включает или выключает повтор текущей песни"],
    ["Формат пианино", "переключает способ управления сустейном через MIDI или MPP"]
];
...
const PRE_REPEAT = PRE_MSG + "[повтор]";
const PRE_SUSTAIN = PRE_MSG + "[Формат пианино]";
const PRE_DOWNLOADING = PRE_MSG + "[Загрузка]";
const PRE_LOAD_MUSIC = PRE_MSG + "[Загрузить музыку]";
const PRE_PUBLIC = PRE_MSG + "[Публичный]";
const PRE_LIMITED = PRE_MSG + "Ограничено!";
const PRE_ERROR = PRE_MSG + "ошибка!";
const WHERE_TO_FIND_MIDIS = "Вы можете найти несколько хороших MIDI-файлов для загрузки с https://bitmidi.com/ и https://midiworld.com/, или вы можете использовать свои собственные MIDI-файлы через Google Drive/Dropbox/и т. д. с прямой ссылкой на скачивание";
...
var createButtons = function() {
    ...
    var repeatTxt = document.createTextNode("повтор");
    ...
    var pauseTxt = document.createTextNode("пауза");
    ...
    var resumeTxt = document.createTextNode("продолжить");
    ...
    var sustainTxt = document.createTextNode("Формат пианино");
    ...
    var publicTxt = document.createTextNode("Публичный");
...
var pause = function() {
    ...
    if (ended) mppChatSend(PRE_PAUSE + ' ' + NO_SONG);
        ...
        if (paused) mppChatSend(title + " The song is already paused");
            ...
            Player.pause();
            paused = true;
            mppChatSend(title + " Пауза " + quoteString(currentSongName));
        }
    }
}
var resume = function() {
...
var sustain = function() {
    ...
    mppChatSend(PRE_SUSTAIN + " Формат пианино установлен на " + (sustainOption ? "MIDI-управление" : "MPP-управление"));
}
...
var public = function(userId, yourId) {
    ...
    mppChatSend(PRE_PUBLIC + " Публичные команды бота были " + (publicOption ? "Включены" : "Отключены"));
}
...
MPP.client.on('a', function (msg) {
    ...
    if (input.startsWith(PREFIX)) {
        ...
        switch (command.toLowerCase()) {
            case "Помощь": case "п": if ((isBotOwner || publicOption) && !preventsPlaying) help(argumentsString, userId, yourId); break;
            case "Информация": case "инфо": if ((isBotOwner || publicOption) && !preventsPlaying) about(); break;
            case "Ссылка": case "сыл-к": if ((isBotOwner || publicOption) && !preventsPlaying) link(); break;
            case "Отзыв": case "от": if (isBotOwner || publicOption) feedback(); break;
            case "ping": case "pi": if (isBotOwner || publicOption) ping(); break;
            case "Играть": case "и": if ((isBotOwner || publicOption) && !preventsPlaying) play(argumentsString); break;
            case "стоп": case "с": if ((isBotOwner || publicOption) && !preventsPlaying) stop(); break;
            case "Пауза": case "па": if ((isBotOwner || publicOption) && !preventsPlaying) pause(); break;
            case "resume": case "r": if ((isBotOwner || publicOption) && !preventsPlaying) resume(); break;
            case "песня": case "so": if ((isBotOwner || publicOption) && !preventsPlaying) song(); break;
